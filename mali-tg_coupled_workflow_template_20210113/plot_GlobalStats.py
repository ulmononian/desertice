...
# -*- coding: utf-8 -*-
"""
Created on Mon Jan 14 14:16:24 2019
...
fig = plt.figure(1, figsize=(9, 8), facecolor='w')
...
#%% added for global stats w/out days since start:
...
def plotStat(fname):
    ...
    f = Dataset(fname,'r')
#    yr = f.variables['daysSinceStart'][:]/365.0 # used for globalStats files w/ days since start
    yr = xtime2numtime(f.variables['xtime'][:]) # used for globalStats files w/out days since start
    ...
    if options.file1inName:
      axVol.plot(yr, vol, 'x-',label=name)
    else:
      axVol.plot(yr, vol, 'o-', label=name)
